...
function getStatusBadge(status: string) {
  switch (status) {
    ...
    case "inactief":
      return <Badge variant="secondary" className="text-[11px] opacity-60">Inactief</Badge>
    default:
      return null
  }
}
...
export function ClientsSection({
  ...
  const gefilterdeClienten = useMemo(() => {
    if (!zoekterm.trim()) return clienten
    const q = zoekterm.toLowerCase()
    return clienten.filter(c =>
      c.naam.toLowerCase().includes(q) ||
      c.email.toLowerCase().includes(q) ||
      c.programma.toLowerCase().includes(q)
    )
  }, [clienten, zoekterm])
    ...
    <div className="flex flex-col gap-6 p-6">
      ...
      <Tabs defaultValue="alle">
        ...
        <TabsContent value="actief" className="mt-4">
          {gefilterdeClienten.filter(c => c.status === "actief").length === 0 ? (
            <EmptyState zoekterm={zoekterm} />
          ) : (
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-3">
        ...
        <TabsContent value="risico" className="mt-4">
          {gefilterdeClienten.filter(c => c.status === "risico").length === 0 ? (
            <EmptyState zoekterm={zoekterm} />
          ) : (
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-3">
        ...
        <TabsContent value="gepauzeerd" className="mt-4">
          {gefilterdeClienten.filter(c => c.status === "gepauzeerd").length === 0 ? (
            <EmptyState zoekterm={zoekterm} />
          ) : (
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-3">
...
function EmptyState({ zoekterm }: { zoekterm: string }) {
  return (
    <div className="flex flex-col items-center justify-center py-12 text-center">
      <Search className="size-8 text-muted-foreground/40 mb-3" />
      <p className="text-sm font-medium text-muted-foreground">
        {zoekterm ? `Geen cliënten gevonden voor "${zoekterm}"` : "Geen cliënten in deze categorie"}
      </p>
