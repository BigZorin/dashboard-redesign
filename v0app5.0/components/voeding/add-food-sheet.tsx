...
const pastMeals = [
  { date: "Zaterdag 22 feb", meals: [
    { name: "Havermout met banaan", kcal: 340, protein: 12, carbs: 58, fat: 8 },
    { name: "Broodje kipfilet", kcal: 380, protein: 32, carbs: 41, fat: 8 },
  ]},
  { date: "Vrijdag 21 feb", meals: [
    { name: "Kwark met fruit", kcal: 220, protein: 28, carbs: 22, fat: 3 },
    { name: "Rijst met kip", kcal: 610, protein: 48, carbs: 84, fat: 4 },
  ]},
]
    ...
    <div className="fixed inset-0 z-50 flex flex-col">
      ...
      <div className={`relative mt-auto w-full max-w-md mx-auto h-[90vh] flex flex-col bg-background rounded-t-3xl overflow-hidden transition-transform duration-500 ease-out ${revealed ? "translate-y-0" : "translate-y-full"}`}>
        ...
        {view === "copy" && (
          <div className="flex-1 overflow-y-auto px-5 pt-3 pb-8">
            <p className="text-[10px] font-semibold uppercase tracking-wider text-[#bad4e1] font-mono mb-5">
              Kopieer naar {mealLabel.toLowerCase()}
            </p>

            {pastMeals.map((day, dayIdx) => (
              <div key={dayIdx} className="mb-5"
                style={{ animationDelay: `${dayIdx * 100}ms`, animation: "fadeSlideUp 0.4s ease-out forwards", opacity: 0 }}>
                <p className="text-xs font-semibold text-muted-foreground font-mono uppercase tracking-wider mb-2">{day.date}</p>
                <div className="flex flex-col gap-2">
                  {day.meals.map((meal, mealIdx) => (
                    <button
                      ...
                      className="flex items-center gap-3 p-3.5 rounded-xl bg-card border border-border text-left transition-all hover:border-[#bad4e1]/30 active:scale-[0.98]"
                    >
                      <div className="h-10 w-10 rounded-lg bg-secondary flex items-center justify-center shrink-0">
                        <Copy className="h-4 w-4 text-muted-foreground" />
                      </div>
                      <div className="flex-1 min-w-0">
                        <span className="text-sm font-semibold text-foreground font-mono block truncate">{meal.name}</span>
                        <p className="text-[11px] text-muted-foreground font-mono mt-0.5">
                          {meal.kcal} kcal | E{meal.protein}g | K{meal.carbs}g | V{meal.fat}g
                        </p>
                      </div>
                      <ChevronRight className="h-4 w-4 text-muted-foreground shrink-0" />
                    </button>
