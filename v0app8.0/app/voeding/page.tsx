...
export default function VoedingPage() {
  ...
  // Staggered add: items appear one by one with a delay between each
  // First item appears after 400ms (to let the sheet close), then every 300ms
  ...
  return (
    <div className="min-h-screen bg-background max-w-md mx-auto relative">
      ...
      <AddFoodSheet
        ...
        onAddItemsStaggered={(items) => {
          const meal = addFoodMeal
          if (meal) handleAddItemsStaggered(meal, items)
        }}
      />
    </div>
  )
